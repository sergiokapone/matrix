...
def generate_all_disciplines(
    ...
    for discipline_code in disciplines.keys():
        ...
        if generate_discipline_page(
            yaml_file, discipline_code, output_file, template_file
        ):
            success_count += 1
...
def main():
    ...
    parser.add_argument("--clean", "-c", action="store_true", help="Очистити каталог перед генерацією")
    ...
    if args.discipline:
        ...
        # Якщо передано --clean, видаляємо каталог з усім вмістом
